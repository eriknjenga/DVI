	...
	$(document).ready(function() { 
		if($('#myTable tr').length>1){
		 ...
		 $("#filter_graph").click(function(){
				...
				var url = '<?php echo base_url();?>wastage_analysis/analysis/'+selected_year+'/'+selected_period; 
				window.location = url;
		});
		$(".district_fetch").click(function(){
			var district = $(this).attr("district");
			var district_name = $(this).attr("district_name");
			var dialog_title = district_name+" District Wastage (by Facility)"
			var selected_year = $("#filter_year").find(":selected").attr("value");
			var selected_period = $("#filter_period").find(":selected").attr("value"); 
			var url = '<?php echo base_url();?>wastage_analysis/district_analysis/'+selected_year+'/'+selected_period+'/'+district;
...
<table id="myTable" class="data-table">
	...
	<tbody>
		...
		foreach($districts as $district_object){
			//Check if this district has any wastage data; if so, display it. If not skip it
			if(isset($district_details[$district_object['ID']]) && $district_object['name'] != ""){?>
				<tr>
					<td><a district = "<?php echo $district_object['ID']; ?>" district_name = "<?php echo $district_object['name']; ?>" class="link district_fetch"><?php echo $district_object['name'];?></a></td>
					<?php 
					//loop through all vaccines to get wastage statistics for each
					foreach($vaccines as $vaccine_object){
						if(isset($district_details[$district_object['ID']][$vaccine_object->id])){?>
							<td><?php echo $district_details[$district_object['ID']][$vaccine_object->id];?></td>
						<?php }
						else{?>
							<td>-</td>
						<?php }
